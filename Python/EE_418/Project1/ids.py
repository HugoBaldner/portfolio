...
class IDS:
    ...
    def estimate_offset(self, a):
        ...
        if self.mode == 'state-of-the-art':
            ...
            #Calulates offset values based on array a
            aarr = []
            # Verify range is 2-N and not 1-N
            for i in range(2, self.N,1):
                aarr.append(a[i]-(a[1]+(i-1)*prev_mu_T_sec))
                
            
            #Computes current average offset     
            curr_avg_offset_us = (1/(self.N-1)*np.sum(aarr))
            test.append(curr_avg_offset_us)
        
            
            #Establish Current Accumulated Offset
            curr_acc_offset_us = (prev_acc_offset_us + np.abs(curr_avg_offset_us))
            ...
            curr_avg_offset_us = self.T_sec - (a[self.N-1] - a0)/self.N
    ...
    def update_clock_skew(self, curr_avg_offset_us, curr_acc_offset_us):
        ...
        #Set lambda to 0.9995
        l = 0.9995
        
        #Establishes G[k]
        G = (l**-1)*prev_P*time_elapsed_sec/(1+(1**-1)*(time_elapsed_sec**2)*prev_P)
        
        #Computes P(k)
        curr_P = (l**-1)*(prev_P-G*time_elapsed_sec*prev_P)
    ...
    def update_cusum(self, curr_error_sample):
        ...
        #Normalizes curr_error_sample
        normalized_error = (curr_error_sample-mu_e)/sigma_e
        
        #Computes curr_L_upper and curr_L_lower  
